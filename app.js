...
function playTrack(index) {
    if (index < 0 || index >= currentPlaylist.length || !player) {
        console.error('PlayTrack Error: Invalid index or player not ready');
        return;
    }
...
function handleTogglePlay() {
    if (!player || currentPlaylist.length === 0) {
        console.log('Cannot toggle play - Player or playlist not ready');
        return;
    }
...
function onPlayerStateChange(event) {
    ...
    if (event.data === YT.PlayerState.PLAYING) {
        ...
        document.getElementById('current-status').textContent = 'Track ended';
        playTrack(currentIndex + 1);
    } else if (event.data === YT.PlayerState.BUFFERING) {
        document.getElementById('current-status').textContent = 'Buffering...';
...
function onPlayerError(e) {
    ...
    switch(e.data) {
        ...
        case 101:
        case 150:
            errorMsg = 'Cannot autoplay video';
            break;
        default:
            errorMsg = 'Playback Error';
    }
    
    document.getElementById('current-status').textContent = errorMsg + ', skipping...';
